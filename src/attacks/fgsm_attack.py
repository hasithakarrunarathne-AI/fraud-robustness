...
BATCH_SIZE = 1024
THRESHOLD = 0.9   # keep same decision threshold as training/evaluation script
...
def attack_fraud_samples_only(model, X_test, y_test, epsilon):
    ...
    for start in range(0, len(X_fraud), BATCH_SIZE):
        ...
        batch_adv = fgsm_attack(model, batch_X, batch_y, epsilon)
...
def compute_asr(clean_preds, adv_preds, y_true, attacked_idx):
    # ASR = among fraud samples that were correctly detected before attack,
    # how many became non-fraud after attack
...
def run_fgsm_attack():
    ...
    #epsilons = [0.001, 0.003, 0.005, 0.01, 0.02]
    ...
    for epsilon in epsilons:
        ...
        X_test_adv, attacked_idx = attack_fraud_samples_only(model, X_test, y_test, epsilon)
    ...
    output_path = "results/attacks/fgsm_results.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=4)

    print(f"\nSaved FGSM results to {output_path}")
